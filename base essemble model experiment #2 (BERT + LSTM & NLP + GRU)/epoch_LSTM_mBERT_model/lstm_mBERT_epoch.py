...
for TOTAL_EPOCHS in EPOCH_LIST:
    ...
    print(f"\nTraining LSTM-mBERT for {TOTAL_EPOCHS} epochs")
    ...
    # ---- Save checkpoint ----
    save_dir = (
        f"essemble model experiment #2\epoch_LSTM_mBERT_model\epoch_{TOTAL_EPOCHS}"
    )
    os.makedirs(save_dir, exist_ok=True)

    torch.save(
        model.state_dict(),
        f"{save_dir}/lstm_bert{TOTAL_EPOCHS}.pt"
    )

    print(f"Saved: {save_dir}/lstm_bert{TOTAL_EPOCHS}.pt")
...
OUTPUT_PATH = (
    "essemble model experiment #2\epoch_LSTM_mBERT_model\LSTM_BERT_Training_Losses.xlsx"
)
