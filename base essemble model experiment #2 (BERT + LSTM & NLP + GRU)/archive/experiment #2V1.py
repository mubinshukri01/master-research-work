...
class LSTM_BERT(nn.Module):
    def __init__(self, num_classes=3, hidden_size=128, num_layers=1, bidirectional=True):
        super().__init__()
...
gru_model.load_state_dict(torch.load("essemble model experiment #2/gru_keras_model.pt"))
gru_model.eval()

lstm_model = LSTM_BERT().to(device)
lstm_model.load_state_dict(
    torch.load("essemble model experiment #2/lstm_bert_model.pt")
)
lstm_model.eval()
...
X_all_keras = torch.load("essemble model experiment #1/keras_sequences.pt")
...
lstm_logits, lstm_labels = collect_logits_lstm(lstm_model, bert_val_loader)
T_lstm = fit_temperature(lstm_logits, lstm_labels)

print("LSTM-BERT temperature:", T_lstm.temperature.item())
...
assert lstm_logits.shape[0] == lstm_labels.shape[0]
...
# ----- Write to Excel -----
OUTPUT_FILE = "essemble model experiment #2/Result/DRSE_Calibrated_Results#2V1.xlsx"
