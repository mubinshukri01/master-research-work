...
# fit only on train texts
keras_tok.fit_on_texts(X_train_texts)

X_keras_train = keras_tok.texts_to_sequences(X_train_texts)
X_keras_train = pad_sequences(X_keras_train, maxlen=MAX_LEN)
X_keras_train = torch.tensor(X_keras_train, dtype=torch.long)
...
opt_cnn = torch.optim.AdamW(cnn.parameters(), lr=2e-5)
...
class BertDataset(Dataset):
    ...
    def __getitem__(self, idx):
        enc = bert_tok(self.texts[idx], padding="max_length", truncation=True, max_length=50, return_tensors="pt")
        return enc["input_ids"].squeeze(0), enc["attention_mask"].squeeze(0), self.labels[idx]

loader = DataLoader(BertDataset(X_train_texts, y_train), batch_size=32, shuffle=True)

# Train CNN+BERT
for epoch in range(5):
    for ids, mask, lab in loader:
        ids, mask, lab = ids.to(device), mask.to(device), torch.tensor(lab).to(device)
        opt_cnn.zero_grad()
        pred = cnn(ids, mask)
...
# OUTPUT_PATH = "essemble model experiment #1/Freeze BERT/DRSE_Ensemble_Results#5V2.xlsx"
